...
# ==========================================
# [기능 8] AI 이미지 자동 태깅 (Auto-Labeling) 📸
# ==========================================

# 1. 딥러닝 모델 로드 (MobileNetV3 - 가볍고 빠름)
print("⏳ 이미지 분류 모델 로딩 중...")
try:
    # 미리 학습된(Pre-trained) 모델을 가져옵니다. (사람이 이미 100만장으로 공부시켜둠)
    classifier = models.mobilenet_v3_small(pretrained=True)
    classifier.eval() # 평가 모드로 설정
    print("✅ 이미지 분류 모델(MobileNet) 장착 완료")
except Exception as e:
    print(f"❌ 모델 로드 실패: {e}")
    classifier = None

# 2. 이미지 전처리기 (모델이 보기 편하게 변환)
...
# 3. ImageNet ID와 우리 카테고리 매핑 (AI는 숫자로 대답하므로 번역 필요)
# ImageNet 클래스 ID: https://deeplearning.cms.waikato.ac.nz/user-guide/class-maps/IMAGENET/
CLASS_MAPPING = {
    # 키보드 관련
    508: "KEYBOARD", # computer keyboard
    509: "KEYBOARD", # typewriter keyboard
...
# ==========================================
# [기능 8] 하이브리드 자동 태깅 (이미지 + 텍스트) 🧠
# ==========================================

# 텍스트에서 카테고리를 유추하는 키워드 사전
KEYWORD_MAPPING = {
    "KEYBOARD": ["키보드", "keyboard", "자판", "축", "스위치"],
    "MOUSE": ["마우스", "mouse", "dpi", "감도"],
    "MONITOR": ["모니터", "monitor", "패널", "inch", "인치", "hz", "qhd", "fhd", "uhd", "ips"],
    "PC": ["본체", "desktop", "데스크탑", "pc", "컴퓨터", "cpu", "gpu", "램", "ssd"],
    "ACC": ["이어폰", "헤드셋", "마이크", "거치대", "스탠드", "케이블", "충전기"]
}

@app.route('/predict-category', methods=['POST'])
def predict_category():
    # 1. 클라이언트가 보낸 데이터 받기
    # 이미지뿐만 아니라 텍스트(이름, 설명)도 같이 받습니다.
    ...
    # ----------------------------------------
    # [Step 1] 텍스트 분석 (Rule-based AI) 📝
    # ----------------------------------------
    full_text = (name + " " + description).lower() # 검색 쉽게 소문자로 통합
        ...
        for category, keywords in KEYWORD_MAPPING.items():
            for keyword in keywords:
                if keyword in full_text:
                    ...
                    break # 찾았으면 루프 탈출
            if text_category: break

    print(f"📝 텍스트 분석 결과: {text_category} (키워드 매칭)")

    # ----------------------------------------
    # [Step 2] 이미지 분석 (Deep Learning AI) 📸
    # ----------------------------------------
    if classifier and file:
        try:
            img_bytes = file.read()
            # OpenCV로 디코딩
            nparr = np.frombuffer(img_bytes, np.uint8)
            ...
            if img_cv is not None:
                ...
                image_prob = top_prob.item() * 100
                image_category = CLASS_MAPPING.get(class_id, "ETC")

                print(f"📸 이미지 분석 결과: {image_category} ({image_prob:.1f}%)")
            else:
                print("❌ 이미지 디코딩 실패 (텍스트 분석에 의존합니다)")
        except Exception as e:
            print(f"❌ 이미지 분석 중 에러: {e}")
    ...
    # 우선순위 1: 텍스트가 가장 정확함 (모델명에 답이 있으니까)
    if text_category:
        final_category = text_category
        method = "TEXT_KEYWORD"
        prob_display = 100.0

    # 우선순위 2: 텍스트가 없거나 모르면 이미지를 믿음
    elif image_category and image_category != "ETC":
        final_category = image_category
        method = "IMAGE_DL"
        prob_display = image_prob
    ...
    return jsonify({
        ...
        "method": method, # 어떤 방식으로 맞췄는지 알려줌
        "msg": f"분석 완료! ({final_category})"
    })
